...
Snapshot database information query

### Command Parameters
```bash
-v|--vm-id=string      Required parameter, Specifies the virtual machine to which the snapshot to be queried belongs, for example: 92815248362
-s|--snap-id=string    Required parameter, Specifies the id of the snapshot to be queried, for example: b81c6fb3-bd1c-4148-93d4-c1301412c55c
-f|--filter=string     Used to specify whether to filter residual_status, lifespan, tpm_files fields, default filtering if not passed, for example: 1
```
...
Error Case:
Error: API return result is not in dictionary format and cannot be parsed! (Reason: The database query interface returned no content)
```