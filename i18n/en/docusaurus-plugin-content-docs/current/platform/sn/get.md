...
```bash
-i|--is-all=integer         Whether to return all serial number info. 1=return all, 0=return basic info only
-g|--get-used=integer       Whether to return usage info. 1=return usage statistics, 0=do not return
-o|--only-nfv=integer       Only fetch nfv authorization info. 1=yes, 0=no
-n|--no-filter-sn=integer   Do not filter serial numbers. 1=yes, 0=no
-s|--sn-modules=string      Authorization data of specified modules; JSON array format required. Example: ['asv','anet']
```

### Usage Example

```bash
acli platform sn get
acli platform sn get --sn-modules '["asv","anet"]'
```
