...
Fix NIC consistency, modify VN and CFS config based on the specified MAC address (Dangerous operation: will overwrite existing configuration)
...
```bash
# Fix NIC MAC address consistency
acli vm nic consistency fix -v 614182240912 -n 0 -m FE:FC:FE:DB:A3:2C
```
